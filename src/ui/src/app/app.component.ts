...
export class AppComponent {
  ...
  constructor(private sensorService: SensorService, private idle: Idle, private welcomeService: WelcomeService) {
    ...
    this.welcomeService.show.subscribe(modalOpen => {
      if (modalOpen) {
        this.idle.stop();
        console.log('[HYPERWALL] Session stopped.');
      } else {
        this.idle.watch();
        console.log('[HYPERWALL] Session started.')
      }
    });

    this.idle.onIdleStart.subscribe(() => {
      console.log('[HYPERWALL] User idle. Timeout started!');
    });

    this.idle.onIdleEnd.subscribe(() => {
      console.log('[HYPERWALL] User no longer idle. Timeout stopped.');
    });

    this.idle.onTimeout.subscribe(() => {
      console.log('[HYPERWALL] User timed out. Resetting...');
